...
sci_b_baud_setting_t g_uart_rc_baud_setting =
        {
        /* Baud rate calculated with 0.019% error. */.baudrate_bits_b.abcse = 0,
...
{ .clock = SCI_B_UART_CLOCK_INT, .rx_edge_start = SCI_B_UART_START_BIT_FALLING_EDGE, .noise_cancel =
  ...
  .rx_fifo_trigger = SCI_B_UART_RX_FIFO_TRIGGER_1, .p_baud_setting = &g_uart_rc_baud_setting, .flow_control =
...
{ .channel = 0, .data_bits = UART_DATA_BITS_8, .parity = UART_PARITY_OFF, .stop_bits = UART_STOP_BITS_1, .p_callback =
          rc_uart_callback,
  .p_context = NULL, .p_extend = &g_uart_rc_cfg_extend,
...
{ .p_ctrl = &g_uart_rc_ctrl, .p_cfg = &g_uart_rc_cfg, .p_api = &g_uart_on_sci_b };
...
void rc_thread_create(void)
{
    ...
    rc_thread = xTaskCreateStatic (
                    ...
                    #endif
                                   rc_thread_func,
                                   (const char*) "RC Input", 2048 / 4, // In words, not bytes
