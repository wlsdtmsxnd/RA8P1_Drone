...
{ .spi_clksyn = SPI_B_SSL_MODE_CLK_SYN,
  ...
  .spck_div =
  {
  /* Actual calculated bitrate: 7812500. */.spbr = 7,
    .brdv = 0 },
...
{ .channel = 1,
  ...
  .bit_order = SPI_BIT_ORDER_MSB_FIRST, .p_transfer_tx = g_spi_imu_P_TRANSFER_TX, .p_transfer_rx =
          g_spi_imu_P_TRANSFER_RX,
...
{ .p_ctrl = &g_spi_imu_ctrl, .p_cfg = &g_spi_imu_cfg, .p_api = &g_spi_on_spi_b };
...
void imu_thread_create(void)
{
    ...
    imu_thread = xTaskCreateStatic (
                    ...
                    #endif
                                    imu_thread_func,
                                    (const char*) "IMU Thread", 4096 / 4, // In words, not bytes
