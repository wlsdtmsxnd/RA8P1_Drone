...
static StaticTask_t uart_thread_memory;
                ...
                static uint8_t uart_thread_stack[2048] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
static uint8_t uart_thread_stack[2048] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.uart_thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
#endif
#endif
...
const rm_freertos_port_parameters_t uart_thread_parameters =
{ .p_context = (void*) NULL, };
...
void uart_thread_create(void)
{
    ...
    uart_thread = xTaskCreateStatic (
                    ...
                    #endif
                                     uart_thread_func,
                                     (const char*) "UART Thread", 2048 / 4, // In words, not bytes
