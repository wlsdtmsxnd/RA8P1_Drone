...
当前初版闭环只用于无桨台架验证，低油门仍保持 1000 us；解锁且油门
离开低位后，四路共同怠速下限为 1150 us，最大输出被限制在 1300 us，
姿态修正限幅很小，不能作为带桨飞行固件。

`TELEMETRY_SOURCE_MOTOR_OUTPUT` 的 VOFA+ 通道定义如下：

- I0～I3：M1～M4 当前输出脉宽（us）。
- I4：归一化油门。
- I5：CH5 Arm 高档标志。
- I6：安全状态 0 DISARMED、1 ARMING_WAIT、2 ARMED、3 FAILSAFE。
- I7：IMU 就绪标志。

测试时仍必须拆桨并固定机架。解锁后保持低油门时 I0～I3 应全部为
1000 us；缓慢推高油门、刚离开低位后四路应同步到约 1150 us，
继续推高后四路基础输出同步升高，但不超过 1300 us。
轻轻右压机架、抬头/低头、左右转动机头时，观察对应电机修正方向是否
抵抗该姿态变化。若任一方向助长倾斜、输出跳变、I6 异常或机架松动，
立即 CH5 拨低并断电，停止后续测试。

Roll 方向的预期修正为：右侧压低时 M2/M3 应增加、M1/M4 应降低；
左侧压低时 M1/M4 应增加、M2/M3 应降低。Pitch 方向的预期修正为：
机头压低时 M1/M2 应增加、M3/M4 应降低；机头抬高时 M3/M4 应增加、
M1/M2 应降低。Yaw 方向需结合实际旋向判断，先记录摇杆右偏时哪一组
对角电机增加，不作为首轮闭环方向通过条件。
